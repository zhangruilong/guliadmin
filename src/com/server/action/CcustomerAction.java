...
public class CcustomerAction extends BaseActionDao {
	...
	public Type TYPE = new TypeToken<ArrayList<Ccustomer>>() {}.getType();
	...
	public void updAll(HttpServletRequest request, HttpServletResponse response){
		...
		if(CommonUtil.isNotEmpty(json)) cuss = CommonConst.GSON.fromJson(json, TYPE);
		for(Ccustomer temp:cuss){
			result = updSingle(temp,CcustomerPoco.KEYCOLUMN);
		}
		responsePW(response, result);
	}
	//导入
	...
	public void expAll(HttpServletRequest request, HttpServletResponse response) throws Exception{
		Queryinfo queryinfo = getQueryinfo(request, Ccustomer.class, CcustomerPoco.QUERYFIELDNAME, CcustomerPoco.ORDER, TYPE);
		cuss = (ArrayList<Ccustomer>) selAll(queryinfo);
		FileUtil.expExcel(response,cuss,CcustomerPoco.CHINESENAME,CcustomerPoco.NAME);
	}
	//查询所有
